...
export default function ProfileSelectInput({Label,Name,types,dispatch,State}:props) {
  return (
    <>
    <div className={styles.selectInput}>
        <label className={styles.label} htmlFor={Label+"-select"}>{Label}</label>
        <select required  name={Name} id={Label+"-select"} onChange={e=>dispatch(Name==="Business_type"?{type:"TYPE",payload:e.target.value}:{type:"Promotion Type",payload:e.target.value})} className={styles.selectType} value={State.type}>
